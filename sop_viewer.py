...
# ==============================================================================
# FUNCIONES AUXILIARES Y GESTIÓN DE SISTEMA
# ==============================================================================
...
class SOPApp(ctk.CTk):
    ...
    def __init__(self):
        ...
        # Ocultar carpeta _internal si existe (al compilar en modo --onedir)
    ...
    def _crear_interfaz(self):
        ...
        self.frame_banner = ctk.CTkFrame(self, corner_radius=8, fg_color="#E6F0FA", height=50)
    ...
    def desplegar_info_y_estandar(self, material):
        ...
        if not df_g.empty:
            row = df_g.iloc[0]
            desc = limpiar_texto(row.iloc[6]) if len(row) > 6 else "Sin descripción"
        ...
        if not df_e.empty:
            ...
            row1 = df_e.iloc[0]
            info_tiempos = (
                f"Pzs/Hr: {limpiar_texto(row1.iloc[1])}  │  1er Turno: {limpiar_texto(row1.iloc[2])}  │  "
                f"2do Turno: {limpiar_texto(row1.iloc[3])}  │  3er Turno: {limpiar_texto(row1.iloc[4])}\n"
                f"Pzs/Ciclo: {limpiar_texto(row1.iloc[5])}  │  Tiempo Ciclo: {limpiar_texto(row1.iloc[6])}  │  "
                f"WIP MAX: {limpiar_texto(row1.iloc[7])}"
            )
    ...
    def _setup_tab_pasos(self):
        ...
        # Leyenda Puntos de Seguridad (6a Hoja)
        self.frame_puntos_seguridad = ctk.CTkFrame(self.tab_pasos, height=110, corner_radius=8, fg_color="#FFF3CD")
        ...
        self.scroll_puntos_seg = ctk.CTkScrollableFrame(self.frame_puntos_seguridad, orientation="horizontal", height=65)
    ...
    def desplegar_pasos_operativos(self, material):
        ...
        # Cargar leyenda de seguridad (Hoja 6)
        ...
        if not self.data_puntos_seg.empty:
            for _, r in self.data_puntos_seg.iterrows():
                ...
                card_seg = ctk.CTkFrame(self.scroll_puntos_seg, fg_color="#FFFFFF", corner_radius=4)
    ...
    def actualizar_vista_paso(self):
        ...
        if total == 0:
            ...
            self.box_paso_desc.insert("1.0", "No hay pasos registrados para este material.")
        ...
        num_paso = limpiar_texto(paso_data.get(self.data_pasos.columns[1])) or str(self.paso_actual_index + 1)
        ...
        self.lbl_paso_nota.configure(text=f"⚠️ Nota: {nota_paso}" if nota_paso else "")
